...
2タイトルともNo-IntroのCRC32が一致し、Snes9xも `Checksum OK` を出す。
**CIC認証もマスタークロックも要らなかった**——必要なのは
「カートに何のクロックも与えないこと」と「速く読み切ること」だけだった。
詳細は [SA-1はクロックを与えなければ読める](#sa-1はクロックを与えなければ読める)。
...
長らく「SA-1はCIC認証を通さないとROMを開示しない」と書いていた。**これは誤りだった。**

    Super Mario RPG (Japan)                      CRC32 5527071e  総和一致
    Hoshi no Kirby Super Deluxe (Japan) (Rev 2)  CRC32 1f35f230  総和一致

どちらも4MBを **95秒** で読み切れる。Snes9xも `ROM+RAM+BAT+SA-1` と認識して
`Checksum OK` を返す。
...
| CIC認証（snesCIC / PIC12F629 / 実チップ） | **不要** |
| Si5351相当のクロック源 | **不要** |
| 1回の接続で全64バンクを読み切る（連続バンクモード） | **必要** |
| 接触の良いカート個体 | **必要** |
| リトライ（窓は確率的） | **必要** |

**なぜ「クロックを与えない」ことが要点なのか。**
Super FXで到達した結論——「クロックが無いから通せないのではなく、
**クロックが無いから大人しかった**」——が、そのままSA-1にも当てはまる。
クロックを与えればSA-1は起きてバス調停を始め、ROMを握ってしまう。
与えなければ眠ったままで、**ROMは素通しで読める**。

MCKを供給していた頃にSA-1が `0x02` や `0x34` を返していたのは、
起きたSA-1がバスを握っていたからだった。**クロックを与えるほど読めなくなっていた。**

**なぜ先人はCICを必要としたのか（推論）。**
sanniのOSCRは 21.477MHz を供給する設計である。与えればSA-1は起きる。
起きたSA-1はCIC認証の成立を待つ。だからOSCRには snesCIC が要る。
**設計選択が、その設計自身に要件を作っていた。**
このリグはクロックを一切与えないので、その要件ごと回避している。

参照実装が間違っていたわけではない。**前提が違うので要求も違った**というだけである。
ただし「先人がやった通りにやる」という方針でその要求ごと輸入し、
CIC認証の実装に何日も費やした。**それは吸い出しには一度も必要なかった。**
...
> **後日の訂正:** ここに書いた苦労は、SA-1の**ROM吸い出しには一度も必要なかった**。
> クロックを与えなければSA-1は眠っていてROMは素通しで読める
> （[SA-1はクロックを与えなければ読める](#sa-1はクロックを与えなければ読める)）。
> CIC認証が要るのは**セーブデータ(BW-RAM)を読むときだけ**である。
> 以下は記録として残す。
