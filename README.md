...
**SA-1（スーパーマリオRPG）は一度だけ開いた。まだ安定して読めない。**
SA-1はROMを開示する前に、自身のCICを介して本物のSNESと通信していることを検証する。
長らく全バイト 0x00 しか返らなかったが、**2026-08-26に一度だけROM窓が開いた**。
条件を詰めきれておらず再現性が無いので、まだ「読める」とは書かない。詳細は
[SA-1に一度だけ届いた記録](#sa-1に一度だけ届いた記録)。
...
長らく「SA-1は読めない」と書いていたが、2026-08-26に**一度だけROM窓が開いた**。
再現できていないので結論ではないが、分かったことは記録しておく価値がある。

**開いたときに取れたもの:**

```
ROM先頭: c2 30 (REP #$30) / 78 (SEI) / a9 00 00 (LDA #$0000) / 5b (TCD) ...
         → 65816のリセットルーチンそのもの
$FFC0:   『SUPER MARIO RPG』 map=0x23(SA-1) chip=0x35 rom=0x0c(4MB) sram=0x05
         checksum=0x4575 補数対OK
バンク$C0を「やや低速」で3回読んで相違0（非ゼロ 61535/65536）
```

**真因は電源だった。** クロックの周波数・供給タイミング・CIC認証の完全性・電源投入
順序と5つの変数を順に潰したが、どれも外れだった。決め手はテスターの1測定で、
カート1番のDC平均が **1.75V** しかなかった。50%デューティの方形波なら2.5Vのはずである。

ただしテスターのDC帯域は数kHz程度しかないので、4MHzでの1.75Vは何の証拠にもならない。
そこでクロックを**約30Hz**（1024分周）まで落として測り直したところ **1.83V**。
帯域の問題ではなく、本当に電圧が足りていなかった。**外部電源を足したら2.55Vになり、
その状態で読んだらROM窓にデータが出た。**

スターフォックスのときと同じ構図である（上記「原因はGSUではなく電源だった」）。
**「効かない」ときは、効かせようとしている信号が本当に出ているかを測る。**

**もう一つの発見: 57番にクロックを出してはいけない。** sanniのSA-1手順は
「システムクロック(57番)にはクロックを出さない」を明示している。57番の駆動を
やめた瞬間、アドレスの偶奇と相関するバスノイズ（`0x83`/`0x00`の交互）が消え、
応答が決定論的になった。

これに伴い、**過去の記録を1つ訂正する。** 以前「MCKを供給したらSA-1が応答した」と
解釈した実測は誤りで、効いていたのは「PHI2への供給をやめた」方だった。当時は
2つの変更を同時に行っており、切り分けられていなかった。

**まだ解けていないこと:** 一度開いたあと施錠状態に戻り、再現できていない。
電源(2.52V維持)・CIC認証(ラウンド0一致)・クロック周波数・慣らし・リセットパルス・
完全な電源再投入をすべて試したが戻らない。CIC認証の成立条件が
「ラウンド0一致」より厳しく、電源投入の巡り合わせに依存している可能性が高い。

なお **CIC認証そのものは必要**である。sanniもSA-1を吸う必須要件として
snesCIC(PIC12F629)の実装を挙げており、nesdevのスレッドでも同様。
