...
読めない」と書いたが、これも誤りだった。詳細は下記。

**SA-1（スーパーマリオRPG）は読めない。** SA-1はROMを開示する前に、自身のCICを介して
本物のSNESと通信していることを検証する。認証が通らない限りバスを握ったまま離さず、
実測でも全バイト 0x00 しか返らなかった。突破にはCICと専用クロックの供給が要り、
本機は対応していない。必要な人は Sanni Cart Reader を参照のこと。
