...
■ 必要なもの（2026-08-31 時点で検証済みの組み合わせ）
    ・**CIC認証**（本体側Lock役をどこかで演じること）
    ・**起動読み**（大きく読む前に1バンクずつ短く読んで起こす）← host/sa1_wake.py
    ・カートに**クロックを一切与えないこと**
    ・1回の接続で全64バンクを読み切ること（4MBで約95秒）

**このスクリプト単体では起動読みをしない。** SA-1を吸うなら host/sa1_wake.py を使うこと。

このスクリプト自体はCIC認証をしない。**別のマイコンが認証を担当している前提**である。
このリポジトリの構成では、Nano-3に焼いた nano3_cicbank が電源投入300ms後に
自力でラウンド0の握手を実行する。カートやコンソールから取り外した実物のCICでも代用できる。

■ なぜクロックを与えないのか
SA-1はROMとバスの間に立つバス調停チップだが、**クロックを与えなければ眠っていて、
ROMは素通しで読める**。Super FXで到達した「クロックが無いから通せないのではなく、
クロックが無いから大人しかった」が、そのままSA-1にも当てはまる。
クロックを与えると起きてバスを掌握し、ROMが読めなくなる。
カート1番(マスタークロック)も57番(システムクロック)も駆動してはいけない。

■ 訂正の記録（2026-08-29）
一時期このコメントに「CIC認証は不要」と書いていた。**誤りだった。**
根拠にした実験は「CIC認証を何度も通したあとにCICなしで試して成功した」というもので、
順序による交絡があった。一度も認証していない状態からCICなしで試すと **0/16**、
同条件でCIC認証を有効にすると **2回目で成功**した。
順序を変えれば結論が変わる実験を、独立した検証として扱っていた。
...
■ 訂正の記録2（2026-08-31）：窓は確率的ではなかった
ここには長く「窓は確率的で、開かなかった回は一色になる」と書いていた。**誤りだった。**

正しくは、**いきなり64バンクを読みに行くとSA-1はROMを出さない**。
先に1バンクずつ短い読み出しを何度か行うと、以後すべての窓が開く。

    起こす前 : $C0 は 0xFF 一色（異なる値1種）
    起動読み : $00 $20 $40 $80 $C0 $E0 を1バンクずつ。**$E0 だけ実データが出る**
    起こした後: $C0/$D0/$E0/$F0 すべて 256種の実データ。64/64バンク成立

45回連続で失敗していたのは運ではなく、手順が足りていなかったためである。
下の prime（バンク$C0のダミーアクセス1回）**だけでは足りない**。
起動読みは host/sa1_wake.py にある。そちらを使うこと。

切り分けの決め手は、正常なLoROMカート（スターフォックス）を挿して $C0 を読んだこと。
**何もマップされていない領域は 0xFF 一色になる**と分かり、
「施錠されている」ではなく「ROMが出ていない」と読み替えられた。
